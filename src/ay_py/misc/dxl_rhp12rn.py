...
class TRHP12RN(object):
  def __init__(self, dev='/dev/ttyUSB0'):
    ...
    self.thg_cmd2pos= lambda cmd: self.thg_range[1] + (cmd-self.CmdMin)*(self.thg_range[0]-self.thg_range[1])/(self.CmdMax-self.CmdMin)
    self.thg_pos2cmd= lambda pos: self.CmdMin + (pos-self.thg_range[1])*(self.CmdMax-self.CmdMin)/(self.thg_range[0]-self.thg_range[1])
    self.thg_cmd2vel= lambda cmd: (self.thg_range[0]-self.thg_range[1])/(self.dxl.ConvPos(self.CmdMax)-self.dxl.ConvPos(self.CmdMin))*self.dxl.ConvVel(cmd)
    self.thg_vel2cmd= lambda vel: self.dxl.InvConvVel(vel*(self.dxl.ConvPos(self.CmdMax)-self.dxl.ConvPos(self.CmdMin))/(self.thg_range[0]-self.thg_range[1]))
  ...
  def Position(self):
    ...
    if pos is None:
      print 'RHP12RNGripper: Failed to read position'
      return None
    pos= self.thg_cmd2pos(pos)
    return pos
  ...
  def Move(self, pos, max_effort=50.0, speed=50.0, blocking=False):
    cmd= max(self.CmdMin,min(self.CmdMax,int(self.thg_pos2cmd(pos))))
  ...
  def StartMoveTh(self):
    self.StopMoveTh()
    with self.port_locker:
      goal_pos= self.thg_cmd2pos(self.dxl.Read('GOAL_POSITION'))
  ...
  def StateObserver(self, callback):
    ...
    while self.threads['StateObserver'][0]:
      with self.port_locker:
        state= {
          'stamp':time.time(),
          'position':self.thg_cmd2pos(self.dxl.Position()),
          'velocity':self.thg_cmd2vel(self.dxl.Velocity()),
